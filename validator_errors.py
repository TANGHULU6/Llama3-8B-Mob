...
def check_consistency(pred_seq, ans_seq, uid, errors):
    # check the consistency between a trajectory in prediction and its counterpart in reference
    error_prefix = "Error regarding uid {}: ".format(uid)
...
def main():
    ...
    if task_id not in task_specs.keys():
        print("Invalid task_id: {}".format(task_id))
        sys.exit(1)
    ...
    uid_set_ref = set(range(*specs["uid_range"]))
    ...
    if uid_set != uid_set_ref:
        extra_uids = uid_set - uid_set_ref  # UIDs in uid_set but not in uid_set_ref
        missing_uids = uid_set_ref - uid_set  # UIDs in uid_set_ref but not in uid_set

        # Print the results
        if extra_uids:
            print(f"Extra UID's in uid_set: {extra_uids}")
        if missing_uids:
            print(f"Missing UID's in uid_set: {missing_uids}")
        error(
    ...
    for uid in range(specs["uid_range"][0], specs["uid_range"][1]):
        pred_seq = pred_uid_dict.get(uid, [])
        ans_seq = ans_uid_dict.get(uid, [])
        check_consistency(pred_seq, ans_seq, uid, errors)
    print("")
